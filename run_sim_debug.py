...
from run_sim_config_debug import jenkins_url,repo_url,build_opt,build_config,\
    copy_to_filer,test_platform,include_sdk,include_release_notes,do_reduced_checkout,\
...
def read_job_params_from_config():
    param_read= {}
    param_read['repo_url'] = repo_url
    param_read['build_opt'] = build_opt
    param_read['copy_to_filer'] = copy_to_filer
    param_read['test_platform'] = test_platform
    param_read['include_sdk'] = include_sdk
    param_read['include_release_notes'] = include_release_notes
    param_read['do_reduced_checkout'] = do_reduced_checkout
    return param_read
...
if build_mode == 1:
    token ='110a917ca15996b0c75702ae40ea33241a'
    j_ser = jenkins.Jenkins(jenkins_url,usernm,token)
    jobs = j_ser.get_jobs()

    # # create job if don't exist
    # if not any(d['name'] == job_name for d in jobs):
    #     j_ser.create_job(job_name, jenkins.EMPTY_CONFIG_XML)
    # jobs = j_ser.get_jobs()
