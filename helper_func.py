...
def create_split(all_files,tr_per,val_per):
    ...
    tr_indx = df.ix[perm[:train_end]]
    val_indx = df.ix[perm[train_end:validate_end]]
    test_indx = df.ix[perm[validate_end:]]
...
# shuffle: if need to shuffle data on every call of function
...
def tr_va_ts_split(bs_pth,tr_per,val_per,shuffle,class_nms,mv):
...
def get_batch_stats(gen,pth_saved_plots):
    ...
    for k,v in gen.class_indices.items():
        ...
        plt.savefig(pth_saved_plots + '/' + 'all_batch_histogram_class_' + k + '.png')
        j = j + 1
...
def save_plots(mdlfit,pth_saved_plots,fnm):
    import matplotlib.pyplot as plt
